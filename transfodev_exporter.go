...
func NewExporter(instances []string, urlPrometheusAPI string) *Exporter {
...
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	...
	for _, instance := range e.instances {
		var upTime = UpTime{}
		computeDailyUpTimePct(&upTime, instance, e.urlPrometheusAPI)
...
*         paramétrer les crédentials
*         gérer les erreurs
*
 */
func computeDailyUpTimePct(uptime *UpTime, instance string, urlPrometheusAPI string) {
	...
	req.SetBasicAuth("admin", "nmv2lzu1n2k1njqklolh")
...
func main() {
	...
	exporter := NewExporter(strings.Split(*instances, ","), *urlPrometheusAPI)
